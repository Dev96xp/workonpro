...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky stashable class="border-r border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            {{-- Apiunta a pagina principal de la aplicación --}}
            <a href="{{ route('home') }}" class="mr-5 flex items-center space-x-2" wire:navigate>
                <x-app-logo class="size-8" href="#"></x-app-logo>
                <span class="text-sm font-semibold text-zinc-900 dark:text-white">Super Admin</span>
            </a>
            ...
            <flux:navlist variant="outline">
                <flux:navlist.group heading="Administración" class="grid">
                    ...
                    <flux:navlist.item icon="credit-card" :href="route('admin.plans.index')" :current="request()->routeIs('admin.plans.*')" wire:navigate>Planes</flux:navlist.item>
            ...
            <flux:dropdown position="bottom" align="start">
                <flux:profile
                    :name="auth('super_admin')->user()->name"
                        ...
                        <div class="p-0 text-sm font-normal">
                            <div class="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
                                <div class="grid flex-1 text-left text-sm leading-tight">
                                    <span class="truncate font-semibold">{{ auth('super_admin')->user()->name }}</span>
                                    <span class="truncate text-xs">{{ auth('super_admin')->user()->email }}</span>
                                </div>
                            </div>
                        </div>
                    </flux:menu.radio.group>

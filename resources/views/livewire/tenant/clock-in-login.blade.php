    ...
    <div class="w-full max-w-sm px-6">
        ...
        <form wire:submit="login" class="space-y-5">
            <flux:field>
                ...
                <flux:input wire:model="email" type="email" autofocus placeholder="empleado@tunegocio.com" />
                <flux:error name="email" />
            </flux:field>

            <flux:field>
                <flux:label>{{ __('tenant.clock_in.password_label') }}</flux:label>
                <flux:input wire:model="password" type="password" />
                <flux:error name="password" />
            </flux:field>

            <flux:button type="submit" variant="primary" class="w-full" wire:loading.attr="disabled">
                <span wire:loading.remove>{{ __('tenant.clock_in.login_button') }}</span>
